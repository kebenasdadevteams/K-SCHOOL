...
type ViewType = 'student' | 'teacher' | 'pastor' | 'editor' | 'admin' | 'developer';

export function SettingsContent({ initialView = 'student' }: { initialView?: ViewType } = {}) {
  const navigate = useNavigate();
  const location = useLocation();
  const { user: authUser } = useAuth();

  const locationState = location.state as { role?: string; userName?: string; userEmail?: string; view?: ViewType } | null;
  const role = locationState?.view ?? initialView;
  const activeView: ViewType = locationState?.view ?? initialView;
...
export default function SettingsPage() {
  ...
  const activeView: ViewType = locationState?.view ?? 'student';

  return (
    <SidebarLayout
      userRole={activeView}
      userName={locationState?.userName || 'Demo User'}
      userEmail={locationState?.userEmail || 'demo@church.com'}
      activeView={activeView}
    >
      <SettingsContent initialView={activeView} />
    </SidebarLayout>
  );
}